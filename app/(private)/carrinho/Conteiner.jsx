...
export default function Carrinho() {
  ...
  return (
    <div className="bg-black text-white min-h-screen">
      {/* Navbar */}
      <nav className="flex justify-between items-center px-4 sm:px-6 lg:px-8 py-3 lg:py-4 border-b border-gray-800 sticky top-0 bg-black/95 backdrop-blur-sm z-50">
        {/* Logo e Menu Mobile */}
        <div className="flex gap-4 items-center">
          ...
          <div className="flex gap-2 items-center">
            <span className="font-bold text-lg sm:text-xl">LOGO</span>
          </div>
        </div>

        {/* Menu Desktop */}
        <div className="hidden lg:flex gap-6 lg:gap-8 items-center">
          <Link href="#" className="hover:text-green-400 transition-colors duration-200 font-medium text-sm lg:text-base">
            HOME
          </Link>
          <Link href="#" className="hover:text-green-400 transition-colors duration-200 font-medium text-sm lg:text-base">
            STORE
        ...
        <div className="flex gap-2 sm:gap-4 items-center">
          ...
          <div className="relative hidden sm:block">
            ...
            <input
              ...
              className="bg-gray-900 border border-gray-700 rounded-full pl-9 pr-4 py-1.5 lg:py-2 text-xs sm:text-sm w-40 sm:w-48 lg:w-64 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all duration-200"
            />
          </div>

          {/* Ícones */}
          <div className="flex gap-1 sm:gap-3 items-center">
            <Link href="/carrinho" className="relative p-1.5 sm:p-2 hover:bg-gray-800 rounded-full transition-colors duration-200 group">
              <ShoppingCart size={18} className="sm:w-5 sm:h-5 group-hover:text-green-400" />
              <span className="absolute -top-1 -right-1 bg-green-500 text-black text-xs rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center font-bold">
                {itens.length}
              </span>
            </Link>
            
            <button className="p-1.5 sm:p-2 hover:bg-gray-800 rounded-full transition-colors duration-200 group">
              <User size={18} className="sm:w-5 sm:h-5 group-hover:text-green-400" />
            </button>
          </div>
        </div>
      </nav>
      ...
      {/* Menu Mobile Overlay */}
      {menuAberto && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black/95 backdrop-blur-sm pt-20">
          <div className="flex flex-col items-center space-y-6 p-6">
            <Link href="#" className="text-xl hover:text-green-400 transition-colors duration-200" onClick={() => setMenuAberto(false)}>
              HOME
            </Link>
            <Link href="#" className="text-xl hover:text-green-400 transition-colors duration-200" onClick={() => setMenuAberto(false)}>
              STORE
      ...
      {/* Search Mobile */}
      <div className="sm:hidden px-4 py-3 border-b border-gray-800">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              ...
              {/* Cabeçalho */}
              <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-800 bg-gray-900">
                <div>
                  <h2 className="text-lg sm:text-xl font-bold mb-1">Seus Produtos</h2>
              ...
              <div className="divide-y divide-gray-800">
                {itens.length === 0 ? (
                  ...
                  <div className="p-6 sm:p-8 lg:p-12 text-center">
                    ...
                    <h3 className="text-lg sm:text-xl font-medium mb-2 sm:mb-3">Seu carrinho está vazio</h3>
                    ...
                    <button 
                      ...
                      className="bg-green-500 hover:bg-green-600 text-black px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-medium text-sm sm:text-base transition-all duration-200"
                    >
                      Explorar Produtos
                    </button>
                  </div>
                  ...
                  // Itens do Carrinho
                  itens.map((item) => (
                    <div key={item.id} className="p-4 sm:p-6 hover:bg-gray-800/50 transition-colors duration-200">
                        ...
                        <div className="flex-shrink-0 relative self-center sm:self-start">
                          <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 bg-gray-800 border border-gray-700 rounded-lg flex items-center justify-center overflow-hidden">
                            <div className="text-center">
                              <Package size={24} className="sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-gray-500 mx-auto" />
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
                            <div className="pr-4 flex-1">
                              <h3 className="text-base sm:text-lg font-semibold mb-1 line-clamp-2">{item.nome}</h3>
                              <div className="flex flex-wrap items-center gap-2 mb-2">
                                <span className="text-gray-400 text-xs sm:text-sm">{item.marca}</span>
                                <span className="text-gray-600 hidden sm:inline">•</span>
                                <span className="text-gray-400 text-xs sm:text-sm">{item.sabor}</span>
                              </div>
                            ...
                            <button
                              ...
                              className="text-red-400 hover:text-red-300 hover:bg-red-400/10 transition-colors p-2 rounded-lg self-end sm:self-start"
                            ...
                            <div className="flex items-center gap-4">
                              <div className="flex items-center border border-gray-700 rounded-lg overflow-hidden bg-gray-800">
                                <button
                                  ...
                                  className="p-1.5 sm:p-2 hover:bg-gray-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                              ...
                              <div className="mt-1 sm:mt-2">
                                {item.estoque <= 5 ? (
                                  <div className="flex items-center text-amber-400 text-xs sm:text-sm">
                                    <AlertCircle size={12} className="sm:w-3 sm:h-3 mr-1" />
                                    Apenas {item.estoque} em estoque
                                  </div>
                                ) : (
                                  <div className="flex items-center text-green-400 text-xs sm:text-sm">
                                    <CheckCircle size={12} className="sm:w-3 sm:h-3 mr-1" />
                                    Em estoque
                                  </div>
                                )}
                              </div>
            ...
            <div className="sticky top-20 lg:top-24">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-800">
                  ...
                  <p className="text-gray-400 text-xs sm:text-sm mt-1">Confirme os valores antes de finalizar</p>
                </div>
                ...
                <div className="p-4 sm:p-6">
                  ...
                  <div className="text-center pt-4 sm:pt-6 border-t border-gray-800 mt-4 sm:mt-6">
                    <p className="text-gray-400 text-xs sm:text-sm mb-2 sm:mb-3">Pagamentos aceitos:</p>
                    <div className="flex justify-center gap-1.5 sm:gap-2 flex-wrap">
                      <div className="bg-gray-800 border border-gray-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded text-xs sm:text-sm">VISA</div>
                      <div className="bg-gray-800 border border-gray-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded text-xs sm:text-sm">MASTERCARD</div>
                      <div className="bg-gray-800 border border-gray-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded text-xs sm:text-sm">TRANSFERÊNCIA</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-medium text-sm sm:text-base mb-0.5 sm:mb-1">Entrega Rápida</h4>
                      <p className="text-gray-400 text-xs sm:text-sm">Envio em 24h para Luanda</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-medium text-sm sm:text-base mb-0.5 sm:mb-1">Compra Segura</h4>
                      <p className="text-gray-400 text-xs sm:text-sm">Pagamento 100% protegido</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-medium text-sm sm:text-base mb-0.5 sm:mb-1">Suporte</h4>
                      <p className="text-gray-400 text-xs sm:text-sm">Ajuda disponível 24/7</p>
                    </div>
                  </div>
                </div>
              </div>
